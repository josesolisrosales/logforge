...
class AnomalyPatternConfig(BaseModel):
    ...
    pattern_type: TemporalPattern = Field(
    ...
    base_rate: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Base anomaly rate (0-1)"
    )
    peak_rate: Optional[float] = Field(
        default=None,
        ge=0.0,
        le=1.0,
        description="Peak anomaly rate for bursts/spikes"
    )
    duration: Optional[str] = Field(
        default=None,
        description="Duration of pattern (e.g., '5m', '1h', '30s')"
    )
    start_time: Optional[str] = Field(
        default=None,
        description="Start time offset (e.g., '10m', '1h')"
    )
    period: Optional[str] = Field(
        default=None,
        description="Period for periodic patterns (e.g., '1h', '1d')"
    )
...
class AnomalyTypeConfig(BaseModel):
    ...
    weight: float = Field(
        default=1.0,
        ge=0.0,
        description="Relative weight for this anomaly type"
    )
    severity_range: tuple[float, float] = Field(
        default=(0.1, 1.0),
        description="Severity range (0-1) for this anomaly type"
    )
    ...
    correlation_groups: List[str] = Field(
        ...
        description="Correlation groups this anomaly belongs to"
    )
...
class AnomalyConfig(BaseModel):
    ...
    seed: Optional[int] = Field(
        default=None,
        description="Random seed for anomaly generation"
    )
    base_rate: float = Field(
        default=0.05,
        ge=0.0,
        le=1.0,
        description="Base anomaly rate across all logs"
    )
    
    # Temporal patterns
    patterns: List[AnomalyPatternConfig] = Field(
        default_factory=list,
        description="Temporal anomaly patterns to inject"
    )
    ...
    correlation_window: str = Field(
        default="5m",
        description="Time window for anomaly correlation"
    )
...
class TimeConfig(BaseModel):
    ...
    end_time: Optional[datetime] = Field(
        default=None,
        description="End time for log generation (defaults to now)"
    )
    ...
    interval: Union[float, str] = Field(
        default=1.0,
        description="Base interval between logs (seconds) or distribution"
    )
    jitter: float = Field(
        default=0.1,
        ge=0.0,
        le=1.0,
        description="Timestamp jitter factor (0-1)"
    )
...
class OutputConfig(BaseModel):
    ...
    compression: Optional[str] = Field(
        default=None,
        description="Compression format (gzip, bz2, lzma)"
    )
    buffer_size: PositiveInt = Field(
        default=65536,
        description="Buffer size for batch writing"
    )
...
class PerformanceConfig(BaseModel):
    ...
    batch_size: PositiveInt = Field(
        default=10000,
        description="Number of logs to generate in each batch"
    )
    workers: Optional[PositiveInt] = Field(
        default=None,
        description="Number of worker processes (None for auto-detect)"
    )
    ...
    use_numpy: bool = Field(
        default=True,
        description="Use numpy for faster array operations"
    )
    precompute_timestamps: bool = Field(
        default=True,
        description="Pre-compute timestamps for better performance"
    )
...
class LogConfig(BaseModel):
    ...
    total_logs: PositiveInt = Field(
        default=1000,
        description="Total number of logs to generate"
    )
    seed: Optional[int] = Field(
        default=None,
    ...
    level_distribution: LogLevelDistribution = Field(
        default_factory=LogLevelDistribution,
    ...
    custom_fields: Dict[str, Any] = Field(
        default_factory=dict,
        description="Custom fields to include in logs"
    )
    ...
    message_templates: Dict[str, List[str]] = Field(
        default_factory=lambda: {
            ...
            "WARNING": ["High memory usage: {memory_percent}%", "Slow query: {query_time}ms"],
    ...
    anomaly_config: AnomalyConfig = Field(
        ...
        description="Anomaly injection configuration"
    )
