...
from logsmith.core.config import (
    AnomalyConfig, AnomalyType, TemporalPattern, 
    AnomalyPatternConfig, AnomalyTypeConfig
)
...
class TemporalPatternGenerator:
    ...
    def __init__(self, config: AnomalyPatternConfig, start_time: datetime, total_duration: timedelta):
        self.config = config
        self.start_time = start_time
        self.total_duration = total_duration
        self.pattern_start_time = self._calculate_pattern_start_time()
        self.pattern_duration = parse_duration(config.duration) if config.duration else total_duration
    ...
    def get_anomaly_rate(self, timestamp: datetime) -> float:
        ...
        if self.config.pattern_type == TemporalPattern.CONSTANT:
            ...
            return self.config.base_rate + (peak_rate - self.config.base_rate) * progress
            ...
            if distance_from_center <= spike_width:
                ...
                return self.config.base_rate + (peak_rate - self.config.base_rate) * spike_intensity
            return self.config.base_rate
            ...
            if self.config.period:
                ...
                cycle_ratio = cycle_progress.total_seconds() / period_duration.total_seconds()
                # Sine wave pattern
                import math
                intensity = (math.sin(2 * math.pi * cycle_ratio) + 1) / 2
                peak_rate = self.config.peak_rate or (self.config.base_rate * 3)
                return self.config.base_rate + (peak_rate - self.config.base_rate) * intensity
            return self.config.base_rate
...
class AnomalyInjector:
    ...
    def __init__(self, config: AnomalyConfig, log_format: str, start_time: datetime, total_duration: timedelta):
        self.config = config
    ...
    def generate_anomaly(self, timestamp: datetime, base_log_data: Dict[str, Any]) -> Optional[AnomalyEvent]:
        ...
        metadata = self._generate_anomaly_metadata(anomaly_type, severity, base_log_data)
    ...
    def _generate_anomaly_metadata(self, anomaly_type: AnomalyType, severity: float, base_log_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if anomaly_type == AnomalyType.HIGH_LATENCY:
            ...
            metadata.update({
                ...
                "auth_method": self.anomaly_random.choice(["password", "token", "certificate", "oauth"]),
            ...
            metadata.update({
                "access_pattern": self.anomaly_random.choice([
                    ...
                    "data_enumeration", "lateral_movement"
                ]),
                "risk_score": severity,
                "source_ip": self._generate_suspicious_ip()
            })
            ...
            metadata.update({
                "service_name": self.anomaly_random.choice([
                    "database", "cache", "auth_service", "payment_api", 
                    "notification_service", "file_storage"
                ]),
    ...
    def _generate_suspicious_ip(self) -> str:
        ...
        suspicious_ranges = [
            "10.0.0.{}", "192.168.1.{}", "172.16.0.{}",  # Internal scanning
            "1.1.1.{}", "8.8.8.{}", "208.67.222.{}",     # Known proxies/DNS
            "185.220.{}.{}", "199.87.{}.{}",             # Known malicious ranges
        ]
        range_template = self.anomaly_random.choice(suspicious_ranges)
        
        if "{}.{}" in range_template:
            return range_template.format(
                self.anomaly_random.randint(1, 254),
                self.anomaly_random.randint(1, 254)
            )
        else:
    ...
    def _handle_correlation(self, anomaly_type: AnomalyType, timestamp: datetime) -> Optional[str]:
    ...
    def apply_anomaly_to_log(self, log_data: Dict[str, Any], anomaly: AnomalyEvent) -> Dict[str, Any]:
    ...
    def _apply_apache_anomaly(self, log_data: Dict[str, Any], anomaly: AnomalyEvent) -> Dict[str, Any]:
    ...
    def _apply_json_anomaly(self, log_data: Dict[str, Any], anomaly: AnomalyEvent) -> Dict[str, Any]:
    ...
    def _apply_syslog_anomaly(self, log_data: Dict[str, Any], anomaly: AnomalyEvent) -> Dict[str, Any]:
