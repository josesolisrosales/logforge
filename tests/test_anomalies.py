...
from logsmith.generators.anomalies import (
    AnomalyInjector, TemporalPatternGenerator, parse_duration, AnomalyEvent
)
...
class TestTemporalPatternGenerator:
    ...
    def test_constant_pattern(self):
        ...
        generator = TemporalPatternGenerator(config, self.start_time, self.total_duration)
...
class TestAnomalyInjector:
    ...
    def test_correlation_handling(self):
        ...
        config = AnomalyConfig(
            enabled=True,
            seed=42,
            correlation_probability=1.0  # Force correlation
        )
        injector = AnomalyInjector(config, "json", self.start_time, self.total_duration)
        ...
        correlation_id1 = injector._handle_correlation(AnomalyType.HIGH_LATENCY, timestamp1)
        ...
        correlation_id2 = injector._handle_correlation(AnomalyType.MEMORY_SPIKE, timestamp2)
    ...
    def test_suspicious_user_agent_generation(self):
        ...
        suspicious_agents = [
            ...
            "sqlmap/1.4.9", "Nmap Scripting Engine"
        ]
